...
export class MeetingWizard {
  ...
  async leave(@Ctx() ctx: any) {
    ...
    console.log({usersToSendMessage});
    ...
    const messages = await Promise.all([
      ...usersToSendMessage.filter(u => typeof u.telegram_id === 'string'||typeof u.telegram_id === 'number').map((user) =>
          ...
          //   :
          {
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    callback_data: `INVITATION_RESULT_ACCEPT_${user.telegram_id}_${+ctx.scene.state.badgeId}`,
                    text: 'Yes',
                  },
                  {
                    callback_data: `INVITATION_RESULT_REJECT_${user.telegram_id}_${+ctx.scene.state.badgeId}`,
                    text: 'No',
                  },
                ],
              ],
